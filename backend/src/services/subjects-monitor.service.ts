...
export class SubjectsMonitorService {
  ...
  async getSubjectsByParties(
    ...
    const partiesVotes = (await this.subjectsRepository
      .createQueryBuilder()
      .select([
        's.id subjectId',
        's.name subjectName',
      ...
      .innerJoin(
        'Voting',
        'v',
        'vs.votingId = v.id AND v.date BETWEEN :start AND :end',
        { start: startDate, end: endDate },
      )
      .innerJoin(
        'VoteByParty',
        'vbp',
        'vbp.votingId = v.id AND vbp.partyId IN (:...partiesIds)',
    ...
    const groupBySubject = partiesVotes.reduce((subjects, party) => {
      if (subjects[party.subjectId] === undefined) {
        subjects[party.subjectId] = new SubjectDTO(
          party.subjectId,
          party.subjectName,
          [],
        );
      ...
      subjects[party.subjectId].votes.push(
        new VoteDTO(
          'party',
          party.entityId,
          party.entityName,
  ...
  async getSubjectsByCongresspeople(
    ...
    // Obs: using custom query because of TypeORM limitations :c
    const congressVotes = (await this.subjectsRepository
      .createQueryBuilder()
      .select([
        's.id subjectId',
        's.name subjectName',
        '"congressperson" entity',
        'c.id entityId',
        'c.name entityName',
        'SUM(vt.yes) yes',
        'SUM(vt.no) no',
      ...
      .innerJoin(
        'Vote',
        'vt',
        'vt.votingId = v.id AND vt.congresspersonId IN (:...congresspersonIds)',
        { congresspersonIds: congresspersonIds },
      )
    ...
    const groupBySubject = congressVotes.reduce((subjects, congressperson) => {
      if (subjects[congressperson.subjectId] === undefined) {
        subjects[congressperson.subjectId] = new SubjectDTO(
          congressperson.subjectId,
          congressperson.subjectName,
          [],
        );
      }

      subjects[congressperson.subjectId].votes.push(
        new VoteDTO(
          'congressperson',
  ...
  async getVotingsByEntities(
    ...
    let partiesSubjects: Map<string, SubjectDTO> =
      await this.getSubjectsByParties(
        partiesIds,
        subjectsIds,
        startDate,
        endDate,
    ...
    Object.keys(partiesSubjects).forEach((key) => {
      if (congressSubjects[key] === undefined) {
        ...
        congressSubjects[key].votes = congressSubjects[key].votes.concat(
          partiesSubjects[key].votes,
        );
      }
    });
