...
export class VotingsMonitorController {
  ...
  constructor(
    private votingMonitorService: VotingsMonitorService,
  ) {}
  ...
  async entitiesActivities(
    ...
    return new VotingsMonitorResponseDTO(
      Object.values(await this.votingMonitorService.getVotingsByEntities(congresspersonIds, partiesIds, startDate, endDate)) 
    );
  }
  ...
  async partiesActivities(
    ...
    return new VotingsMonitorResponseDTO(
      Object.values(await this.votingMonitorService.getVotingsByParties(partiesIds, startDate, endDate)) 
    );
  }
  ...
  async congresspeopleActivities(
    ...
    return new VotingsMonitorResponseDTO(
      Object.values(await this.votingMonitorService.getVotingsByCongresspeople(congresspersonIds, startDate, endDate)) 
    );
  }
