...
export class SubjectsMonitorController {
  ...
  async entitiesActivities(
    ...
    const subjectsIds = (await this.subjectService.getFilteredByName(subjects, regexSubjects)).map((s) => s.id)

    return new SubjectsMonitorResponseDTO(
      Object.values(await this.subjectsMonitorService.getVotingsByEntities(congresspersonIds, partiesIds, subjectsIds, startDate, endDate)) 
    );
  }
  ...
  async partiesActivities(
    ...
    return new SubjectsMonitorResponseDTO(
      Object.values(await this.subjectsMonitorService.getSubjectsByParties(partiesIds, subjectsIds, startDate, endDate)) 
    );
  }
  ...
  async congresspeopleActivities(
    ...
    return new SubjectsMonitorResponseDTO(
      Object.values(await this.subjectsMonitorService.getSubjectsByCongresspeople(congresspersonIds, subjectsIds, startDate, endDate)) 
    );
  }
